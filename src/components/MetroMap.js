...
const MetroMap = () => {
  // Get all the paths with class "line"
  var lines = document.querySelectorAll(".line");

  // Add event listeners for each line
  lines.forEach(function (line) {
    var originalStrokeWidth = parseFloat(line.style.strokeWidth) || 3;

    line.addEventListener("mouseover", function () {
      // Increase stroke width on hover
      line.style.strokeWidth = originalStrokeWidth + 3 + "px";
    });

    line.addEventListener("mouseout", function () {
      // Revert to original stroke width on mouseout
      line.style.strokeWidth = originalStrokeWidth + "px";
    });

    line.addEventListener("click", function () {
      // Show an alert box on click
      alert("You clicked on " + line.id);
    });
  });
  return (
